...
## Security
Load `../../roles/security/ROLE.md`, `../../roles/security/RUBRIC.md`, and `../../roles/security/references/security-review-workflow.md` first. Identify the language/frameworks in the approved slice, then load every matching file from `../../roles/security/references/` before judging the diff. In this skill, use Security as a reviewer-only exploitability/trust-boundary adapter: check secrets, auth, injection, unsafe parsing/deserialization, external sends, data exposure, and privilege boundaries when the issue is exploitability or trust-boundary regression. Findings must include severity, impact, `file:line` evidence, suggested fix, owning implementer role (`backend` or `frontend`), and re-review trigger; fixes route back to the implementer and Security re-reviews.
