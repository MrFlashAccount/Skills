...
```bash
node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs next --lease-token <token> --run-id <run-id> [--workflow <workflow.json>] [--user-prompt <text> | --user-prompt-file <path>]
node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs write-output --lease-token <token> --run-id <run-id> --step-id <id> [--json <json>] [--workflow <workflow.json>]
node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs continue --lease-token <token> --run-id <run-id> [--workflow <workflow.json>]
node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs instructions --lease-token <token> --run-id <run-id> --step-id <id>
```
...
```json
{
  ...
  "orchestratorInstruction": "Supersedes all previous workflow-runner stdout.\nExecute every host request in this JSON and wait until all requested actions finish: [{\"id\":\"step_id\",\"stepId\":\"step_id\",\"action\":\"run_worker\",\"loadInstructionsCommand\":\"node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs instructions --run-id 'run_id' --step-id 'step_id' --lease-token <lease-token>\"}]\nThen run:\nnode skills/orbita/lib/entrypoints/cli/workflow-runner.mjs continue --run-id 'run_id' --lease-token <lease-token> --only-instructions\nFollow that stdout instruction exactly.",
  "baton": {},
  "requests": [
    {
      "id": "step_id",
      "stepId": "step_id",
      "action": "run_worker",
      "loadInstructionsCommand": "node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs instructions --run-id 'run_id' --step-id 'step_id' --lease-token <lease-token>"
    }
  ]
}
```
...
```bash
node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs write-output --lease-token "$WORKFLOW_RUN_TOKEN" --run-id "$RUN_ID" --step-id "step_id" --workflow "$WORKFLOW" <<'JSON'
...
```bash
node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs continue --lease-token "$WORKFLOW_RUN_TOKEN" --run-id "$RUN_ID" --workflow "$WORKFLOW" --only-instructions
```
...
`skills/orbita/lib/entrypoints/cli/start-run.mjs` is legacy initialization/inspection only. It requires the same lease token carrier and never exposes private baton/history paths. It does not accept `--user-prompt` or `--user-prompt-file`; use `workflow-runner next` for startup prompt capture and instruction rendering.
