...
- `skills/orbita/lib/entrypoints/` owns CLI/API parsing and host-facing response shape. It resolves run arguments, calls persistence, invokes use-cases, and returns instruction text or JSON responses.
- `skills/orbita/lib/persistence/` owns filesystem/resource IO: workflow files, baton/run-state files, instruction files, templates, output schemas, role material, durable commits, and host-request storage paths.
- `skills/orbita/lib/use-cases/` owns process orchestration across entities. `RunNext`, `ContinueRun`, `ApplyWorkflowOutput`, `LoadInstructions`, `ValidateWorkflow`, and `InspectWorkflow` receive DTO/raw boundary data, construct entities, call entity methods, and return boundary results without direct filesystem access.
- `skills/orbita/lib/file-contracts/workflow-document-schema.mjs` and `workflow-document.json` own the external workflow document/file contract.
- `skills/orbita/lib/entities/Workflow/index.mjs` owns workflow semantic validation, topology, step lookup, transition semantics, output schema semantics, and baton cursor inference.
- `skills/orbita/lib/entities/Baton/index.mjs` owns runtime state/cursor/status consistency, pending-output access, and safe state updates.
- `skills/orbita/lib/entities/Step/index.mjs` owns step-level input projection, transition descriptors, concrete target resolution, output application intent, instruction-request validation, and render-context preparation.
- `skills/orbita/lib/entities/Template/index.mjs` plus `skills/orbita/lib/entities/Template/compiler/**` own prompt/template rendering mechanics.
- `skills/orbita/lib/dtos/` owns boundary shapes only: workflow, baton/run-state, step, template, instruction, output, and workflow-result DTOs.
...
```bash
node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs next --lease-token <token> --run-id <run-id> --workflow <workflow.json> [--diagnostics]
```
