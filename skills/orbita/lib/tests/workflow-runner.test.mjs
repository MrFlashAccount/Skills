...
test('runner: next returns a single host action request with load command only', () => {
  ...
  assert.match(response.orchestratorInstruction, new RegExp(`Then run:\\nnode skills/orbita/lib/entrypoints/cli/workflow-runner\\.mjs continue --run-id '${runId}' --lease-token '${leaseToken}' --only-instructions`));
  ...
  assert.equal(response.requests[0].loadInstructionsCommand, `node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs instructions --run-id '${runId}' --step-id 'prepare' --lease-token '${leaseToken}'`);
  ...
  assert.equal(lastResponse.requests[0].loadInstructionsCommand, `node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs instructions --run-id '${runId}' --step-id 'prepare' --lease-token <lease-token>`);
...
test('runner: non-next modes reject empty user prompt file option', () => {
  ...
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /usage: node skills\/orbita\/lib\/entrypoints\/cli\/workflow-runner\.mjs/);
});
