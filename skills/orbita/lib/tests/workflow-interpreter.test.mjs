...
test('cli: removed directive alias is rejected', () => {
  ...
  const result = runNode(['skills/orbita/lib/entrypoints/cli/workflow-interpreter.mjs', 'directive', wfPath, batonPath]);
...
test('cli: positional paths may begin with dash', () => {
  ...
  const result = runNode(
    [path.join(root, 'skills/orbita/lib/entrypoints/cli/workflow-interpreter.mjs'), 'inspect', '--workflow.json', '--baton.json'],
    tempDir,
  );
...
test('cli: three-position apply without explicit mode is rejected', () => {
  ...
  const result = runNode(['skills/orbita/lib/entrypoints/cli/workflow-interpreter.mjs', wfPath, batonPath, outputPath]);

  assert.equal(result.status, 1);
  assert.match(
    result.stderr,
    /workflow-interpreter: usage: node skills\/orbita\/lib\/entrypoints\/cli\/workflow-interpreter\.mjs inspect <workflow\.json> <baton\.json> \| render \[--diagnostics\] <workflow\.json> <baton\.json> \| apply <workflow\.json> <baton\.json> <worker-output\.json>/,
  );
...
test('cli: schema validation rejects wrong arity with mode-specific usage', () => {
  const inspect = runNode(['skills/orbita/lib/entrypoints/cli/workflow-interpreter.mjs', 'inspect', 'workflow.json']);
  const apply = runNode(['skills/orbita/lib/entrypoints/cli/workflow-interpreter.mjs', 'apply', 'workflow.json', 'baton.json']);
  const missingMode = runNode(['skills/orbita/lib/entrypoints/cli/workflow-interpreter.mjs', 'workflow.json', 'baton.json']);

  assert.equal(inspect.status, 1);
  assert.match(inspect.stderr, /workflow-interpreter: usage: node skills\/orbita\/lib\/entrypoints\/cli\/workflow-interpreter\.mjs inspect <workflow\.json> <baton\.json>/);

  assert.equal(apply.status, 1);
  assert.match(apply.stderr, /workflow-interpreter: usage: node skills\/orbita\/lib\/entrypoints\/cli\/workflow-interpreter\.mjs apply <workflow\.json> <baton\.json> <worker-output\.json>/);

  assert.equal(missingMode.status, 1);
  assert.match(missingMode.stderr, /workflow-interpreter: usage: node skills\/orbita\/lib\/entrypoints\/cli\/workflow-interpreter\.mjs inspect <workflow\.json> <baton\.json> \| render \[--diagnostics\] <workflow\.json> <baton\.json> \| apply <workflow\.json> <baton\.json> <worker-output\.json>/);
});

test('cli: unknown explicit mode with apply arity is rejected by argument schema', () => {
  const result = runNode(['skills/orbita/lib/entrypoints/cli/workflow-interpreter.mjs', 'bogus', 'workflow.json', 'baton.json', 'output.json']);
