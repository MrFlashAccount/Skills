...
test('cli: three-position apply without explicit mode is rejected', () => {
  ...
  assert.match(
    result.stderr,
    /workflow-interpreter: usage: node \.\/lib\/entrypoints\/cli\/workflow-interpreter\.mjs inspect <workflow\.json> <baton\.json> \| render \[--diagnostics\] <workflow\.json> <baton\.json> \| apply <workflow\.json> <baton\.json> <worker-output\.json>/,
  );
...
test('cli: schema validation rejects wrong arity with mode-specific usage', () => {
  ...
  assert.match(inspect.stderr, /workflow-interpreter: usage: node \.\/lib\/entrypoints\/cli\/workflow-interpreter\.mjs inspect <workflow\.json> <baton\.json>/);

  assert.equal(apply.status, 1);
  assert.match(apply.stderr, /workflow-interpreter: usage: node \.\/lib\/entrypoints\/cli\/workflow-interpreter\.mjs apply <workflow\.json> <baton\.json> <worker-output\.json>/);

  assert.equal(missingMode.status, 1);
  assert.match(missingMode.stderr, /workflow-interpreter: usage: node \.\/lib\/entrypoints\/cli\/workflow-interpreter\.mjs inspect <workflow\.json> <baton\.json> \| render \[--diagnostics\] <workflow\.json> <baton\.json> \| apply <workflow\.json> <baton\.json> <worker-output\.json>/);
});
