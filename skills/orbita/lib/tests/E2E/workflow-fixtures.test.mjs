...
test('E2E fixture: long happy path loops through review revision and preserves latest state', () => {
  ...
  assert.doesNotMatch(planned.orchestratorInstruction, /ORBITA APPROVAL READABLE VIEW/);
  assert.match(planned.orchestratorInstruction, /Approval request: approval_gate/);
  assert.match(planned.orchestratorInstruction, /Show this approval request to the user\./);
  assert.match(planned.orchestratorInstruction, /Show\/send these required artifacts or files to the user before asking for approval:/);
