...
test('renderWorkflowPrompt injects provided validating writer command into output schema instructions', () => {
  const rendered = renderWorkflowPrompt({
    ...
    resources: {
      ...resources,
      validatingWriterCommand: "node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs write-output --run-id example --step-id consumer --lease-token example-token <<'JSON'\n<paste strict JSON here>\nJSON",
    },
  });
