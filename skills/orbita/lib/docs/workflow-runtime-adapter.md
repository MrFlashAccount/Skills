...
The readable view is not a broad presentation API and not a request-schema migration. Host adapters may render it to users, but machine routing still comes from `requests[]`, the inline request JSON directive, `write-output`, and `continue --only-instructions`.
...
Approval output without a declared schema is any host/user JSON object compatible with the approval transition. The approval-readable-view block must show the minimal normalized approval form:
...
- `wait_for_approval` does not spawn a worker. The wrapper/orchestrator follows the inline approval prompt and additive approval-readable-view block in runner stdout, attaches required-read files and projected artifact paths through the approval mechanism, shows the required context/artifacts to the user, asks only for the requested decision/input, and writes the normalized approval JSON with the validating writer command from that prompt. The wrapper must not inline full artifact bodies into stdout by default.
