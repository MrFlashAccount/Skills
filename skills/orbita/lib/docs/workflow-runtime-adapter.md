...
- `wait_for_approval` does not spawn a worker. The wrapper/orchestrator follows the approval protocol in runner stdout, attaches the listed projected artifacts through the approval mechanism, renders the provided `<message>` block to the user, asks only for the requested decision/input, and writes the normalized approval JSON with the validating writer command from that protocol.
