...
- `wait_for_approval` does not spawn a worker. The wrapper/orchestrator follows the inline approval prompt in runner stdout, attaches required-read files and projected artifact paths through the approval mechanism, shows the required context/artifacts to the user, asks only for the requested decision/input, and writes the normalized approval JSON with the validating writer command from that prompt.
