...
`orchestratorInstruction` is a machine-visible directive for the host/orchestrator. Runner stdout is an active directive, not durable history: each new `next` or `continue --only-instructions` stdout supersedes every previous runner stdout. When `status` is `needs_host_actions`, the host must treat the response as non-terminal: finish every request in the inline JSON request array, run the embedded `continue --only-instructions` command, and follow the next directive returned by runner stdout. `run_worker` requests are executed by starting or restoring a worker/subagent with a command string selected from the request: use `loadFollowupInstructionsCommand` only when the host can continue or restore the opaque `preferredAgentId`, otherwise use `loadInstructionsCommand` for a fresh worker. The selected command is embedded in the strict worker bootstrap and watchdog policy from `skills/orbita/SKILL.md`; the full bootstrap text intentionally lives in the skill instead of every runner response. After the host knows the actual selected worker id, it runs `bindAgentCommand` after replacing only the literal `<agent-id>` placeholder with the shell-quoted worker id; this writes only `baton.workerBindings[stepId]` and does not use `write-output`. `wait_for_approval` requests are executed by the orchestrator itself; approval requests must not carry `preferredAgentId`, `bindAgentCommand`, or `loadFollowupInstructionsCommand`. The runner stdout includes one compact approval request message for each current approval request. That message is the default user-facing source: it names the requested decision, tells the host to show/send the listed required artifacts or files to the user before asking for approval, includes the normalized answer contract, and includes the validating `write-output` command. The shared `needs_host_actions` stdout tail remains responsible for the exact `continue --only-instructions` command after every current request output is accepted. The approval request message does not inline the full compiled approval prompt, projected baton state, or artifact bodies by default. The full compiled prompt remains available through the request `loadInstructionsCommand` for diagnostics or when a listed artifact/reference is unclear. When required-read files or projected artifact paths are present, the host must attach or link those files through the approval mechanism instead of replacing them with summaries or inline full artifact bodies; if attachment/linking is unavailable, the approval message must state that capability gap and name the affected path/reference. Only `done` and `blocked` are terminal; terminal `--only-instructions` stdout includes minified terminal response JSON with `status` and `baton` so the host has final result or blocker details without falling back to full JSON mode.

### Approval request message

The approval request message is the compact stdout affordance for humans reviewing `wait_for_approval` requests. It belongs inside `orchestratorInstruction` next to the existing approval request directive, not inside `requests[]`, baton state, history, artifact storage, or `write-output` validation.

Each approval request message must:

- be emitted only for current `wait_for_approval` requests;
- identify the approval request id and step name/id;
- preserve the existing parseable current-request JSON directive in stdout;
- explicitly tell the host to show the approval request to the user;
- explicitly tell the host to show/send required artifacts or files to the user before asking for approval;
- show the exact validating `write-output` command expected for the approval answer;
- name required-read paths and projected artifact references as references or attachable paths;
- keep the raw compiled approval prompt and projected baton state out of stdout by default;
- keep full artifact bodies out of stdout by default;
- state the normalized answer shape for schema-less approvals, at minimum `{ "approval": "approved" }`, and include any supported rejection/request-changes shape when the workflow contract supports it;
- defer schema-backed answer validation and normalization to the declared `output.schema` plus `write-output`.

The approval request message is not a broad presentation API and not a request-schema migration. Host adapters may render it to users, but machine routing still comes from `requests[]`, the inline request JSON directive, `write-output`, and `continue --only-instructions`.
...
Approval output without a declared schema is any host/user JSON object compatible with the approval transition. The approval request message must show the minimal normalized approval form:
...
- `wait_for_approval` does not spawn a worker. The wrapper/orchestrator follows the compact approval request message in runner stdout, attaches or links required-read files and projected artifact paths through the approval mechanism, shows/sends the required context/artifacts to the user, asks only for the requested decision/input, and writes the normalized approval JSON with the validating writer command from that message. The wrapper must not inline the raw compiled approval prompt, projected baton state, or full artifact bodies into stdout by default.
