...
For `wait_for_approval`, the orchestrator handles the request directly. Follow the approval instruction in the latest runner stdout exactly. The approval instruction is an executable protocol, not a worker prompt: attach every artifact listed under `Attach these artifacts`, render the `<message>` block to the user as the final message, normalize the user's answer to the strict JSON shape described by the instruction, and run the validating `write-output` command printed there. Do not inline full artifact bodies as a replacement for attachments. If the host cannot attach or link a listed artifact, state that capability gap in the user message and include the path/reference that could not be attached. When user approval or input blocks the next step, put the full approval/request message in final, not commentary, and do not send a separate short final after it. Treat accepted output as completion of that host request, then continue following the latest `next`/`continue` stdout instruction.
