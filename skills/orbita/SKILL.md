---
name: orbita
description: Use Orbita when driving workflow-runner runs from the repo root through runner-returned next/continue --only-instructions directives, host requests, bounded workers, and user-input waits.
---
...
The skill entrypoint is `skills/orbita`, but the workflow-runner CLI still lives under `skills/orbita/lib`.
...
```bash
node skills/orbita/lib/entrypoints/cli/workflow-runs.mjs list
```
...
```bash
node skills/orbita/lib/entrypoints/cli/workflow-runs.mjs create --workflow <workflow> --title '<title>' --summary '<summary>' --owner <owner> --harness <harness> --session-id <session-id>
```

Claim the selected run before calling the runner:

```bash
lease_token=$(node skills/orbita/lib/entrypoints/cli/workflow-runs.mjs claim --run-id <run-id> --owner <owner> --harness <harness> --session-id <session-id> --print-lease-token)
```
...
```bash
node skills/orbita/lib/entrypoints/cli/workflow-runner.mjs next --run-id <run-id> --user-prompt '<clear dense user task prompt>' --lease-token "$lease_token" --only-instructions
```
