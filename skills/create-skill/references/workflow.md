...
If a critic/reviewer pass is delegated to a worker/subagent, role label alone is not enough. The parent prompt must include the selected role/phase overlay and require the worker to load the selected role material before reviewing. The worker must follow all instructions in loaded role material, including loading additional role material, references, rubrics, learnings, task guidance, [references/checklist.md](checklist.md), and relevant create-skill workflow/testing references when required. If loaded role material defines final-answer requirements, the worker must satisfy them. Do not accept the critic/reviewer pass for a required gate when required material cannot be loaded or final-answer requirements cannot be satisfied.
