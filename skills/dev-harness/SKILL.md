...
- Delegated role/worker prompts must require `role_files_loaded` listing loaded `ROLE.md`, `RUBRIC.md`, and adapter/reference files, or `blocked` if any required role file could not be loaded. Do not accept delegated Architect, Planner, implementer, or reviewer output for a required gate when that evidence is absent or wrong.
