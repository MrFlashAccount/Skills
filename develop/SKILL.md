...
- `scripts/persist-run-state.mjs` — persist returned run state.
...
7. If the workflow interpreter `apply` succeeds, persist the returned state before continuing; otherwise follow the failure behavior below.
...
8. Loop until `directive.action` is `stop_done` or `stop_blocked`, or until the workflow interpreter returns a blocker. Store the final summary/blocker output in the run directory.
