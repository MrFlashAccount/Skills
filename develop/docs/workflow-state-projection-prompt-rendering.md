...
- `develop/lib/dtos/schemas/workflow.json` requires flat workflow documents (`name`, `version`, `start`, `done`, `blocked`, `steps` at the JSON root), rejects wrapped workflow documents, rejects step-level extension fields, and allows workflow-scoped extensions only.
