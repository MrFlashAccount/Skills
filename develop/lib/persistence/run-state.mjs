...
import { isInside } from '../entities/Template/path-utils.mjs';
...
export async function ensureRunStorage(paths) {
  await mkdir(paths.runDir, { recursive: true });
  await mkdir(paths.runnerDir, { recursive: true });
  await mkdir(paths.instructionsDir, { recursive: true });
}

export async function managedJsonFileExists(path, name) {
  const fileExists = await exists(path);
  if (fileExists) await assertManagedFileIsNotSymlink(path, name);
  return fileExists;
}

export async function createJsonFileIfMissing(path, value) {
  return createFileIfMissing(path, `${JSON.stringify(value, null, 2)}\n`);
}

export async function createHistoryFileIfMissing(paths) {
