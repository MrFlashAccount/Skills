...
function publicWorkflowError(message) {
  return { ok: false, mode: 'run', openclaw_surface: PLUGIN_ID, workflow: 'dev-harness', error_code: message, message };
}
...
function resolveSubagentCallable(api) {
  const subagent = api?.runtime?.subagent;
  if (!subagent) return undefined;
  if (typeof subagent === 'function') return (request) => subagent(request);
  if (typeof subagent.run === 'function') return (request) => subagent.run(request);
  if (typeof subagent.spawn === 'function') return (request) => subagent.spawn(request);
  if (typeof subagent.create === 'function') return (request) => subagent.create(request);
  return undefined;
}

async function callRuntimeSubagent(api, request) {
  const callable = resolveSubagentCallable(api);
  if (!callable) throw new Error('runtime_subagent_unavailable');
  const result = await callable(request);
  return parseWorkerOutput(result);
}
...
function workerPrompt({ instructions, runId, stepId, artifactDir }) {
...
function approvalText(response) {
...
function terminalText(response) {
  if (response.status === 'done') return '🪐 DevHarness workflow completed.';
  if (response.status === 'blocked') return '🪐 DevHarness workflow blocked.';
  return '🪐 DevHarness workflow stopped.';
}
...
async function executeWorkerRequest({ api, response, request, workflowPath, runsRoot, leaseToken }) {
  ...
  const output = await callRuntimeSubagent(api, {
    label: `orbita-dev-harness-${stepId}`,
    task: workerPrompt({ instructions, runId: response.runId, stepId, artifactDir }),
    prompt: workerPrompt({ instructions, runId: response.runId, stepId, artifactDir }),
    cwd: REPO_ROOT,
    cleanup: 'delete',
  });
...
export async function runDevHarnessWorkflow(values = {}, { pluginConfig = {}, ctx = {}, api } = {}) {
  ...
  if (!resolveSubagentCallable(api)) return publicWorkflowError('runtime_subagent_unavailable');
  ...
  try {
    ...
    for (let index = 0; index < MAX_WORKFLOW_STEPS; index += 1) {
      ...
      if (approval) return { ...safeRunProjection(response, { text: approvalText(response) }), text: approvalText(response) };
      if (response.status === 'done' || response.status === 'blocked') return { ...safeRunProjection(response, { text: terminalText(response) }), text: terminalText(response) };
      if (requests.length !== 1 || requests[0]?.action !== 'run_worker') return publicWorkflowError('unsupported_workflow_host_action');
      await executeWorkerRequest({ api, response, request: requests[0], workflowPath, runsRoot, leaseToken });
      response = await continueRun({ runId: response.runId, workflowPath, leaseToken, runsRoot });
    }
    return publicWorkflowError('workflow_step_limit_exceeded');
  } catch {
    return publicWorkflowError('dev_harness_workflow_failed');
  }
}
