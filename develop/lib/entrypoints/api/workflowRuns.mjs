...
import { assertSafeRunId, defaultWorkflowPath, resolveRunPaths, workflowRunsRoot } from '../../persistence/run-state/paths.mjs';
import { createRunIndexEntry, readRunsIndex, runsIndexPathsForRoot, updateRunIndexEntry } from '../../persistence/run-state/run-index.mjs';
...
export async function listWorkflowRuns({ runsRoot = workflowRunsRoot, now = new Date() } = {}) {
...
export async function registerWorkflowRun({ runId, title, summary, workflowPath, workflowIdentity, status = 'running', taskKey, taskFingerprint, runsRoot = workflowRunsRoot, claim = false, owner, harness, sessionId, workerId, leaseMs, now = new Date() } = {}) {
...
export async function claimWorkflowRun({ runId, workflowPath, runsRoot = workflowRunsRoot, owner, harness, sessionId, workerId, leaseMs, now = new Date() } = {}) {
