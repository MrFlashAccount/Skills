...
test('Orbita public help and tool schema expose only approved lifecycle modes', () => {
  ...
  assert.deepEqual(tool.parameters.properties.mode.enum, ['run', 'inbox', 'status', 'list', 'cancel', 'help']);
  ...
  assert.deepEqual(cliModes, ['run', 'inbox', 'status', 'list', 'cancel', 'help']);
});
...
test('Orbita registered tool blocks persistent modes without trusted requester context', async () => {
  ...
  for (const mode of ['run', 'inbox', 'status', 'list', 'cancel']) {
